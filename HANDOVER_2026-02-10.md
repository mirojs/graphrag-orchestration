...
- [ ] **Contextual header stacking** — ~~prepend section_path to chunk text before embedding~~ ✅ Implemented 2026-02-17: label prefix `[Document: X | Section: Y]` prepended to both sentence and chunk text **for embedding only** (stored text stays raw). See `_build_label_prefix()` and `_chunk_label_prefix()` in `lazygraphrag_pipeline.py`. Requires re-indexing to take effect.
