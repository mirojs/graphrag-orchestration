...
### 0. CRITICAL: Deploy + re-benchmark Route 7 after PPR score bug fixes
- **Two critical bugs fixed (§33.18):**
  - Bug 4: PPR scores were hardcoded to 1.0 — synthesizer couldn't distinguish relevant from irrelevant chunks
  - Bug 5: Chunk order from Neo4j was random — PPR ranking lost
- **These bugs meant Route 7 achieved 57/57 (Q-D) and 52/57 (Q-G) WITHOUT its core ranking signal.** The PPR pipeline's output was discarded before synthesis.
- **Action:** Deploy, re-benchmark Q-D (regression check — expect 57/57 maintained), re-benchmark Q-G (improvement target — currently 52/57)
- **Commit:** `08e1025`
