...
Scenarios
- hybrid_route2_detailed_report: response_type=detailed_report (LLM synthesis)
- hybrid_route2_audit_trail:     response_type=audit_trail (LLM synthesis with audit info)
- hybrid_route2_summary:         response_type=summary (LLM synthesis, concise)
- hybrid_route2_nlp_audit:       response_type=nlp_audit (NLP extraction, 100% deterministic, no LLM)
- hybrid_route2_nlp_connected:   response_type=nlp_connected (NLP extraction + temperature=0 rephrasing)
...
def _read_question_bank(path: Path, *, prefix: str = "Q-L") -> List[BankQuestion]:
    """Read local questions from question bank (Q-L prefix)."""
...
def main():
    ...
    # Read local questions (Q-L prefix)
    questions = _read_question_bank(qbank_path, prefix="Q-L")
    print(f"Loaded {len(questions)} local questions from {qbank_path.name}")

    # Define scenarios
    scenarios = [
        {"name": "hybrid_route2_detailed_report", "response_type": "detailed_report"},
        {"name": "hybrid_route2_audit_trail", "response_type": "audit_trail"},
        {"name": "hybrid_route2_summary", "response_type": "summary"},
        {"name": "hybrid_route2_nlp_audit", "response_type": "nlp_audit"},
        {"name": "hybrid_route2_nlp_connected", "response_type": "nlp_connected"},
    ]
    ...
    out_json = out_dir / f"route2_repeatability_{timestamp}.json"
    out_md = out_dir / f"route2_repeatability_{timestamp}.md"
