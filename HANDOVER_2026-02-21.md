...
- **Q-G4 (1/3):** Route 7 missed PMA monthly statement reporting obligation. Triple embeddings don't encode "reporting" concepts well. **Action:** Test `ROUTE7_COMMUNITY_SEEDS=1` + `ROUTE7_W_COMMUNITY=0.8` on Q-G benchmark with Q-D regression check (see §33.15 of architecture doc).
- **Q-G6 (1/3) — ANALYSIS COMPLETE (§33.16):** Root cause is synthesis layer, not retrieval. Both routes correctly retrieved all 4 expected parties but synthesizer also included incidental entity mentions (invoice recipients, form copyright owners, etc.) that are in the documents but not contracting parties. Fix options: (A) `prompt_variant="party_enumeration"` in synthesis prompt, (B) indexing-time `is_contracting_party` tagging, (C) revise ground truth to be less narrow.
...
### 4. Phase 2 ablation on Q-G questions
- Phase 2 features (structural seeds, community seeds) may help Route 7 on global questions where vanilla Phase 1 scored 52/57. Worth testing once Q-G eval baseline is confirmed. Primary target: Q-G4 via elevated community seed weight.

### 5. Remaining Route 5 issues (from Session 1)
