...
Key design principles (informed by Voyage / industry research):
  - Metadata-driven context: Each sentence carries parent_paragraph_text, prev/next
    sentence IDs so retrieval can do "sentence search, paragraph display"
  - Voyage already understands sentence boundaries within paragraphs — no explicit
    markers needed. We embed whole paragraphs and query at sentence level.
  - Deterministic edges (NEXT, PREV, PART_OF) are NEVER generated by k-NN.
    Only RELATED_TO edges use similarity, with strict budget controls.
  - Hallucination control: Stage 3 re-ranking uses grounding checks, pointwise
    relevance scoring, and confidence gating before sending context to LLM.
  - Edge budget: We track total edges vs sentence count to ensure O(n) not O(n²).
...
Requirements:
    pip install neo4j numpy voyageai python-dotenv
"""
...
FORM_LABEL_RE = re.compile(
    r"^(name|date|address|phone|email|signature|title|page|total|amount|number|id|ref|no\.?)\s*[:_\-]?\s*$",
    re.IGNORECASE,
)
...
class SentenceNode:
    ...
    source: str  # "paragraph", "table_row", "language_span"
...
def _is_noise_sentence(text: str) -> bool:
    """Filter out DI noise: headers, form labels, table cells, fragments."""
    text = text.strip()
    if len(text) < MIN_SENTENCE_CHARS:
        return True
    if len(text.split()) < MIN_SENTENCE_WORDS:
        return True
    if FORM_LABEL_RE.match(text):
        return True
...
def _extract_sentences_from_chunk(chunk: Dict) -> List[Dict[str, Any]]:
    """Extract sentence units from chunk metadata.

    Uses the same hybrid strategy described in the analysis:
    - DI paragraphs as base units (from chunk text, paragraph-split)
    - Table-row linearizations from metadata
    - Filtered language_spans for sub-paragraph granularity
    """
    sentences = []
    ...
    # --- Source A: Split chunk text into paragraphs (double newline) ---
    paragraphs = [p.strip() for p in re.split(r'\n\s*\n', chunk_text) if p.strip()]
    para_idx = 0
    for para in paragraphs:
        # Further split long paragraphs into sentences
        sent_texts = re.split(r'(?<=[.!?])\s+', para)
    ...
    # --- Source B: Table-row linearizations from metadata ---
    table_sentences = metadata.get("sentences", [])
    if isinstance(table_sentences, list):
        for ts in table_sentences:
            if isinstance(ts, dict):
                text = ts.get("text", "")
                source = ts.get("source", "")
                if source == "table" and text and not _is_noise_sentence(text):
                    sentences.append({
                        "text": text,
                        "source": "table_row",
                        "index": para_idx,
    ...
    # --- Source C: language_span sentences (filtered) ---
    # These provide sub-paragraph granularity but are noisy
    if isinstance(table_sentences, list):
        for ts in table_sentences:
            if isinstance(ts, dict):
                text = ts.get("text", "")
                source = ts.get("source", "")
                confidence = ts.get("confidence", 0.9)
                if source != "table" and text and confidence >= 0.5 and not _is_noise_sentence(text):
                    # Check if this sentence is already covered by paragraph extraction
                    already_covered = any(
                        s["text"] == text or text in s["text"]
                        for s in sentences if s["source"] == "paragraph"
                    )
                    if not already_covered:
                        sentences.append({
                            "text": text,
                            "source": "language_span",
                            "index": para_idx,
                            "page": ts.get("page"),
