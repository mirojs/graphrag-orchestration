...
Usage in lazygraphrag_pipeline.py:
    ...
    from src.worker.hybrid_v2.indexing.section_chunking.integration import (
...
from src.worker.hybrid_v2.services.neo4j_store import TextChunk
...
async def chunk_di_units_sliding_window(
    di_units: Sequence[Any],
    doc_id: str,
    doc_source: str = "",
    doc_title: str = "",
    ...
    Concatenates all DI unit text, splits into sentences via spaCy, then
    creates overlapping 3-sentence windows (stride=1).  Each window becomes
    a TextChunk (passage node) for the PPR graph.

    Falls back to section-aware chunking if spaCy produces zero sentences.
    """
    from .sliding_window import chunk_sliding_window_3sentence

    # Concatenate all DI unit text
    full_text = "\n\n".join(
    ...
    section_chunks = chunk_sliding_window_3sentence(
        full_text=full_text,
        doc_id=doc_id,
        doc_source=doc_source,
        doc_title=doc_title,
    ...
    if not section_chunks:
        # Fallback: if spaCy produces no sentences, use section-aware
        logger.warning(
            "sliding_window_fallback_to_section_aware",
            extra={"doc_id": doc_id, "text_len": len(full_text)},
        )
        return await chunk_di_units_section_aware(
            di_units=di_units, doc_id=doc_id,
            doc_source=doc_source, doc_title=doc_title,
            doc_language=doc_language,
        )

    text_chunks = section_chunks_to_text_chunks(section_chunks)

    # Propagate document-level metadata (same pattern as section-aware)
    _DOC_LEVEL_KEYS = ("signature_block", "barcodes", "figures", "selection_marks")
    if text_chunks:
        for unit in di_units:
            first_meta = getattr(unit, "metadata", None) or {}
...
def get_summary_chunks(text_chunks: List[TextChunk]) -> List[TextChunk]:
    ...
    Usage in enhanced_graph_retriever.py:
        from src.worker.hybrid_v2.indexing.section_chunking.integration import get_summary_chunks
