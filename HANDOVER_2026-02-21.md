...
### 0. CRITICAL: Deploy + re-benchmark Route 7 after PPR score bug fixes — **DONE**
- **Two critical bugs fixed and deployed (§33.18, commit `08e1025`):**
  - Bug 4: PPR scores hardcoded to 1.0 — now propagated correctly to synthesizer
  - Bug 5: Chunk order from Neo4j was random — now sorted by PPR score descending
- **Post-fix results (2026-02-22):**
  - Q-D: **56/57** (was 57/57 — Q-D8 dropped 3/3→2/3, minor imprecision in document set overlap)
  - Q-G: **51/57** (was 52/57 — Q-G7 dropped 3/3→2/3)
- **Key finding:** PPR ranking *helps* Q-D but *hurts* Q-G — PPR score is not correlated with clause-enumeration relevance. The "all-equal" pre-fix behavior accidentally gave better Q-G coverage. This confirms Route 7 ≠ universal replacement for Route 6. Both routes serve distinctly different retrieval needs.
- **No further action needed on bug fix — results documented in §33.18**

### 1. Investigate Route 3/6 Q-G score history
- **Clarification:** No 57/57 exists for Route 3/6 on Q-G in git history. Previous Route 3 benchmarks used **theme coverage** (keyword matching), not LLM judge. Theme coverage 100% ≠ LLM judge 3/3 — Route 7 also got 100% theme coverage on Q-G6 but scored 1/3 from LLM judge.
- **Action:** Run LLM judge on Route 3 to get fair comparison with Route 6 (53/57) and Route 7 (52/57)

### 2. Route 7 Q-G improvement (after bug fix re-benchmark)
- **Q-G4 (1/3):** Test `ROUTE7_COMMUNITY_SEEDS=1` + `ROUTE7_W_COMMUNITY=0.8` — but re-benchmark with bug fixes first to establish new baseline
- **Q-G6 (1/3) — ANALYSIS COMPLETE (§33.16):** Synthesis layer issue. Fix options documented.

### 3. Assess Route 7 as universal route replacement
- **HippoRAG 2 research findings (§33.17):**
  - Paper only tests entity-relational + narrative QA; global/thematic/aggregation/temporal questions NOT evaluated
  - Known community issues: PPR sometimes doesn't change rankings (#166), entity-dense docs cause false matches (#154), 18% zero-triple fallback rate
  - Route 7 MAY replace Route 2 (local search) — paper shows best-in-class on simple factual QA
  - Route 7 replacing Route 6 (global search) needs bug-fix re-benchmark first; if Q-G improves significantly, consolidation is viable
  - **Consolidating to 1-2 routes would save significant infrastructure cost**

### 4. New frontier question bank (Q-A, Q-T, Q-I, Q-C)
- 20 new questions added to question bank Section G targeting synthesis gaps
- Need benchmark scripts updated to support these new question prefixes
- These questions test the LLM, not the retrieval — run on Route 7 to establish baseline
