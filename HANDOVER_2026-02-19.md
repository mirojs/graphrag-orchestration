...
## 5. Community Detection Fix (Completed)

### Root cause
Step 8 (Louvain via GDS) had already run — all 363 entities have `community_id` (56 distinct communities). Step 9 (materialize Community nodes) never ran because the gate at `lazygraphrag_pipeline.py:437` checks `stats.get("gds_communities", 0) > 0`, and the stat wasn't propagated.

### Fix applied
Created `scripts/materialize_communities.py` — standalone script that:
1. Groups entities by existing `community_id` on Entity nodes
2. Creates Community nodes with entity-derived titles (no LLM dependency)
3. Creates BELONGS_TO edges
4. Embeddings auto-generated by `community_matcher._ensure_embeddings()` at query time

Run: `python scripts/materialize_communities.py --group test-5pdfs-v2-fix2 --clean`

Result: **27 Community nodes**, **332 BELONGS_TO edges** created.

### Tier contribution — before vs after

| Tier | Before (avg) | After (avg) |
|------|-------------|-------------|
| T1 (NER) seeds | 7.7 | 8.6 |
| T1 mass | 0.787 | 0.629 |
| T2 (Structural) seeds | 37.0 | 36.4 |
| T2 mass | 0.579 | 0.428 |
| **T3 (Community) seeds** | **0.0** | **15.2** |
| **T3 mass** | **0.000** | **0.315** |

NER-empty queries (Q-D3, Q-D7, Q-G6, Q-G10) now get T3 mass=0.4 — community matching is the primary seed source for these concept-anchored queries.

### LLM summaries (optional improvement)
Communities currently use entity-name-based titles (fallback embedding). Running with `--generate-summaries --openai-endpoint <url>` would add LLM-generated semantic summaries for higher quality community embeddings.

---

## 6. Commits Pushed (2026-02-19)
...
|----------|------|--------|
| ~~**P0**~~ | ~~Fix community indexing~~ | **Done** — 27 communities materialized |
| ~~**P0**~~ | ~~Re-evaluate Tier 3 after communities are indexed~~ | **Done** — T3 avg 15.2 seeds, 0.315 mass |
| **P0** | Build Route 6 for concept search (from Route 3 foundation) | Architecture decided, not started |
| **P0** | Generate LLM summaries for communities | Script ready, needs Azure OpenAI endpoint |
| **P1** | Wire BM25/fulltext search into seed resolver | Designed, not implemented |
| **P1** | Reclassify routing: entity/concept vs global/drift | Depends on Route 6 |
| **P2** | Demote NER to exact-match-only boost | Depends on Route 6 evaluation |
| **P2** | Add Sentence→Entity edges for finer-grained PPR seeds | Requires pipeline change |
