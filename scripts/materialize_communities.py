...
# ---------------------------------------------------------------------------
# Step 5 (optional): Clear stale embeddings
# ---------------------------------------------------------------------------
def clear_community_embeddings(driver, group_id: str) -> int:
    """Clear embedding and embedding_text_hash from all Community nodes.

    This forces community_matcher._ensure_embeddings() to regenerate
    embeddings using the current summary text on the next query-time load.
    Necessary after summaries are updated (otherwise old fallback embeddings
    persist and produce near-zero cosine similarity scores).
    """
    count_query = """
    MATCH (c:Community {group_id: $group_id})
    ...
    clear_query = """
    MATCH (c:Community {group_id: $group_id})
    SET c.embedding = null, c.embedding_text_hash = null
    """
    with driver.session() as session:
        cnt = session.run(count_query, group_id=group_id).single()["cnt"]
        if cnt > 0:
            session.run(clear_query, group_id=group_id)
    return cnt
...
def main():
    ...
    parser.add_argument("--regenerate-embeddings", action="store_true",
                        help="Clear existing embeddings (forces re-embedding on next query load)")
    ...
    # Step 4: Clear stale embeddings (auto after summaries, or explicit flag)
    embeddings_cleared = 0
    if not args.dry_run and (summaries_generated > 0 or args.regenerate_embeddings):
        reason = "new summaries generated" if summaries_generated > 0 else "explicit --regenerate-embeddings"
        print(f"Step 4: Clearing stale embeddings ({reason})...")
        embeddings_cleared = clear_community_embeddings(driver, args.group)
        print(f"  Cleared embeddings from {embeddings_cleared} communities")
        print("  Embeddings will be regenerated on next query-time load via _ensure_embeddings().")
        print()
    ...
    if not args.dry_run:
        ...
        print(f"  Embeddings cleared:        {embeddings_cleared}")
        print()
        if embeddings_cleared > 0:
            print("Note: Embeddings will be regenerated with current summary text")
            print("      on next query-time load via _ensure_embeddings().")
        else:
