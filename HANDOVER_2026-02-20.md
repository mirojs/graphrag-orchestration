...
The fundamental difference between v1 and v2 is **where seeds come from**:

- **HippoRAG 1** — seeds are found by running NER on the *query* at retrieval time. The LLM extracts entity mentions from the question text and those become PPR seed nodes. This is fragile: query phrasing varies, the LLM can miss entities or produce name variants that don't exactly match graph nodes, and short queries give the NER little to work with.

- **HippoRAG 2** — seeds are derived from **knowledge graph triplets** (subject-predicate-object triples) extracted during *ingestion*. The graph already contains normalized, linked entity nodes. Seeding from triplets means the PPR starts from nodes that are guaranteed to exist in the graph with consistent naming, making retrieval deterministic and reliable regardless of how the query is phrased.

Our custom implementation follows the v2 triplet-seeding design. The `seed_resolver.py` pipeline resolves seeds across three tiers before PPR runs:
- **Tier 1** — direct entity matches from the query (NER + vector search)
- **Tier 2** — structural seeds: Section nodes + SignatureBlock sentence nodes → linked entities
- **Tier 3** — expanded graph neighbourhood seeds

This means even if Tier 1 NER misses "Contoso Ltd." in the query, Tier 2 seeds it from the signature block triplets already in the graph.
