#!/usr/bin/env python3
"""
Index 5 Test PDFs with V2 Voyage Embeddings + Enhanced Extraction Examples

This script tests the improved entity extraction with invoice-specific examples.
Tests whether adding examples for product models, URLs, and prices improves extraction.

Usage:
    python3 scripts/index_5pdfs_v2_enhanced_examples.py --group-id test-5pdfs-v2-enhanced-ex
"""
...
async def run_v2_indexing(group_id: str, reindex: bool, dry_run: bool, knn_enabled: bool = False):
...
def main():
    parser = argparse.ArgumentParser(description="V2 Enhanced Examples Testing")
    parser.add_argument("--dry-run", action="store_true", help="Verify setup without indexing")
    parser.add_argument("--group-id", type=str, help="Group ID (default: test-5pdfs-v2-enhanced-ex)")
    ...
    # Use fixed group ID for testing
    group_id = args.group_id or "test-5pdfs-v2-enhanced-ex"
    reindex = True  # Always reindex for testing
    knn_enabled = False  # Disable KNN to focus on extraction quality
    
    log(f"\n🧪 Testing Enhanced Extraction Examples")
    log(f"   Group: {group_id}")
    log(f"   KNN: Disabled (focusing on extraction quality)")
    log("")
    
    # Run indexing
    stats = asyncio.run(run_v2_indexing(group_id, reindex, args.dry_run, knn_enabled))
