...
from src.worker.hybrid.services.entity_deduplication import EntityDeduplicationService
from src.worker.hybrid.services.neo4j_store import Document, Entity, Neo4jStoreV3, Relationship, TextChunk
from src.worker.hybrid.utils.language import canonical_key_for_entity, is_cjk, detect_cjk_from_text
...
class LazyGraphRAGIndexingPipeline:
    ...
    async def _chunk_di_units_section_aware(self, *, di_units: Sequence[LlamaDocument], doc_id: str) -> List[TextChunk]:
        ...
        from src.worker.hybrid.indexing.section_chunking.integration import chunk_di_units_section_aware
    ...
    async def _process_di_metadata_to_graph(
        ...
        *,
        ...
        Args:
            ...
            expanded_docs: List of document dicts with di_extracted_docs
        ...
        for doc in expanded_docs:
            ...
            with self.neo4j_store.driver.session(database=self.neo4j_store.database) as session:
                if barcodes:
                    barcode_data = [{
                        "id": self._stable_barcode_id(group_id, bc.get("value", "")),
                        "group_id": group_id,
                ...
                if figures:
                    figure_data = [{
                        "id": self._stable_figure_id(group_id, fig.get("id", "")),
                        "group_id": group_id,
                        "di_id": fig.get("id", ""),
                    ...
                    stats["figures_created"] += result.single()["count"]
                    
                    ref_edges = []
                    for fig in figures:
                        fig_id = self._stable_figure_id(group_id, fig.get("id", ""))
                        for ref in fig.get("element_refs", []):
                            ref_edges.append({"figure_id": fig_id, "ref_path": ref.get("ref", "")})
